...
def add_tracks(access_token, num_tracks, time_range, user_id, owner_id, playlist_id, now):
    ...
    try:
        for x in xrange(0, tt_json['limit']):
            ...
            try:
                db_insert(
                    "INSERT INTO tracks (user_id,track_id,playlist_id,date,the_key,image_url) VALUES (%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE date=values(date)",
                    (user_id, track_id, playlist_id, now, the_key, track_image))
...
def create_playlist(access_token, user_id, title):
    ...
    cp_post = {'name': title, 'public': 'true', 'collaborative': 'false',
               'description': 'created at http://mp.soundshelter.net'}
