...
def process():
    ...
    try:
        db_insert(
            "INSERT INTO user (user_id,email,join_date,last_visit,source,ref_code,the_key) VALUES (%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE last_visit=VALUES(last_visit), the_key=VALUES(the_key);",
            (user_id, email, now, now, 'new', referall_code, access_token))
        print "Inserted user"
    ...
    # no referrer to check to see if they have an existing playlist
        ...
        check_for_pl = db_select(
            "SELECT user_id,playlist_id FROM participation WHERE user_id=%s AND time_range=%s LIMIT 1",
            (user_id, time_range))
        if check_for_pl.rowcount == 0:
            ...
            up_headers = {'Authorization': access_token, 'Accept': 'application/json',
    ...
    db_insert(
        "INSERT INTO participation (user_id,playlist_id,the_key,time_range) VALUES (%s,%s,%s,%s) ON DUPLICATE KEY UPDATE the_key=VALUES(the_key)",
        (user_id, playlist_id, the_key, time_range))
